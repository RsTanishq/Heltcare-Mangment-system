    ...
    <>
      <Card className="w-full">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-medium">Patient Directory</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {filteredPatients.map((patient) => (
                  <TableRow key={patient.id}>
                    ...
                    <TableCell>{getStatusBadge(patient.lastVisit || '')}</TableCell>
                ...
                <div className="space-y-4">
                  {selectedPatient.medicalHistory.map((history, index) => (
                    <div key={index} className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600">Diagnosed: {history.diagnosedDate}</p>
                      {history.medications && (
                        <p className="text-sm">Medications: {history.medications.join(", ")}</p>
                      )}
                      {history.notes && (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p><span className="font-medium">Recent BP:</span> {selectedPatient.recentBloodPressure}</p>
                  </div>
                  <div>
                    <p><span className="font-medium">Highest BP:</span> {selectedPatient.highestBloodPressure}</p>
                    <p><span className="font-medium">Lowest BP:</span> {selectedPatient.lowestBloodPressure}</p>
                  </div>
                </div>
              </div>
            </div>
